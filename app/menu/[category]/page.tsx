    ...
    <main className="flex flex-wrap text-primary min-w-screen">
      {pizzas.map((pizza) => (
        <Link href={`/product/${pizza.id}`} key={pizza.id}
          className='w-full sm:w-1/2 h-[60vh] lg:h-[50vh] lg:w-1/3 border-r-2 border-secondary border-b-2 p-2  flex flex-col justify-between group'>
          {/* IMAGE CONTAINER */}
          {pizza.img && (
          ...
          {/* TEXT CONTAINER */}
          <div className="flex justify-between items-center font-bold p-2 uppercase text-xl md:text-2xl xl:text-3xl ">
